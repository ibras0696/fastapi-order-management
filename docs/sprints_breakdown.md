...
Важные правила
1 Тестировать делать отладку через курл и тесты с пайплайнами через пайтест

2 Коменты

3 не использовать хардкодинг

4 не использовать магию если можно использовать то что есть

5 Делать максимально удобные мейкфайлы
...
class RateLimiter:
    ...
    Класс реализует алгоритм "ведра токенов" для ограничения скорости выполнения действий.
    Безопасен для использования в рамках одного процесса (worker).
    В распределённой среде состояние следует хранить в общем хранилище
    (например, Redis).
    ...
    refill_rate : float
        Количество токенов, добавляемых в секунду.
    tokens : float
        Текущее количество доступных токенов.
    last_refill_ts : float
        Временная метка последнего пополнения (в секундах с начала эпохи).

    Notes
    -----
    Реализация использует системное время. При резком изменении системных часов
    поведение пополнения токенов может быть некорректным.
    ...
    def allow(self, cost: int = 1) -> bool:
        """Проверить, разрешено ли выполнение действия.

        Parameters
        ----------
        cost : int, default=1
            Стоимость действия в токенах.

        Returns
        -------
        bool
            True, если доступно достаточное количество токенов,
            иначе False.

        Raises
        ------
        ValueError
            Если cost не является положительным числом.
        """
        if cost <= 0:
